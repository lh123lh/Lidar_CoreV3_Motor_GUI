...
pub fn get_stream<R: Read>(reader: &mut R) -> std::io::Result<Vec<u8>> {
...
/// Turns timeout errors into `Ok(None)`
pub fn get_stream_timeout<R: Read>(reader: &mut R) -> std::io::Result<Option<Vec<u8>>> {
  match get_stream(reader) {
      Ok(buf) => Ok(Some(buf)),
      Err(err) => {
          if err.kind() == io::ErrorKind::TimedOut {
              Ok(None)
